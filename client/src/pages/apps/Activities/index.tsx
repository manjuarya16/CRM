...
const ActivitiesPage: React.FC = () => {
  ...
  const fetchActivities = async () => {
    try {
      setLoading(true);
      const res = await API.get(`/activities?page=${page}&per_page=${perPage}&search=${search}`).catch(() => ({ data: { data: [] } }));
    ...
    <div className="p-6 space-y-6">
      ...
      <div className="bg-white dark:bg-gray-800 rounded-xl shadow-sm border border-gray-100 dark:border-gray-700 overflow-hidden">
        <div className="p-4 border-b border-gray-100 dark:border-gray-700 flex flex-wrap items-center justify-between gap-4">
          <form onSubmit={(e) => { e.preventDefault(); fetchActivities(); }} className="flex items-center gap-2">
            <input
              type="text"
              value={search}
          ...
          <div className="flex items-center gap-4 text-sm text-gray-600 dark:text-gray-300">
            ...
            <span>{activities.length > 0 ? `${(page - 1) * perPage + 1} - ${Math.min(page * perPage, total)} of ${total}` : `0 - 0 of ${total}`}</span>
          </div>
        </div>
        ...
        <div className="overflow-x-auto">
          <table className="w-full text-left border-collapse text-sm">
            ...
            <tbody>
              {loading ? (
                <tr><td colSpan={7} className="text-center py-12 text-gray-400"><div className="inline-block animate-spin rounded-full h-6 w-6 border-2 border-[#0088cc] border-t-transparent"></div></td></tr>
              ) : activities.length === 0 ? (
                <tr><td colSpan={7} className="text-center py-14 text-gray-400 dark:text-gray-500 text-sm font-medium">No Records Available.</td></tr>
              ) : (
                activities.map((act) => (
                  <tr key={act.id} className="border-b border-gray-100 dark:border-gray-700/60 hover:bg-gray-50/60">
                    <td className="py-3 px-4 font-medium text-[#0088cc]">{act.title || "-"}</td>
                    <td className="py-3 px-4">{act.type}</td>
                    <td className="py-3 px-4 text-gray-500">{act.schedule_from || "-"}</td>
                    <td className="py-3 px-4 text-gray-500">{act.schedule_to || "-"}</td>
                    <td className="py-3 px-4"><span className={`px-2 py-0.5 text-xs rounded ${act.is_done ? "bg-green-50 text-green-600" : "bg-amber-50 text-amber-600"}`}>{act.is_done ? "Done" : "Pending"}</span></td>
                    <td className="py-3 px-4">{act.location || "-"}</td>
