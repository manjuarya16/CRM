...
const PersonsPage: React.FC = () => {
  ...
  const formatEmailsText = (emails: any): string => {
    const list = parseItems(emails, "work");
    if (!list.length) return "-";
    return list.map((item) => `${item.value}${item.label ? ` (${item.label})` : ""}`).join(", ");
  };

  const formatContactsText = (contacts: any): string => {
    const list = parseItems(contacts, "work");
    if (!list.length) return "-";
    return list.map((item) => `${item.value}${item.label ? ` (${item.label})` : ""}`).join(", ");
  };
  ...
  const handleExport = () => {
    ...
    const rows = persons.map((p) => [
      p.id,
      `"${(p.name || "").replace(/"/g, '""')}"`,
      `"${formatEmailsText(p.emails).replace(/"/g, '""')}"`,
      `"${formatContactsText(p.contact_numbers).replace(/"/g, '""')}"`,
